...
| 接口 | 用途 | 前提 |
| --- | --- | --- |
| `PlayerModel.Snapshot` | 读取位置、朝向、潜行、伪装、生命与存活状态 | `PlayerRules.Reset` 已执行；`PlayerModel.cs:18` |
| `PlayerRules.ApplyDamage(in DamageIntent)` | 施加正整数伤害，生命最低为 0 | 从容器取 `PlayerRules`；`PlayerRules.cs:86` |
| `PlayerRules.Step(in PlayerIntent, float)` | 固定 tick 内推进并返回是否攻击 | 只由遭遇逻辑步骤调度；`PlayerRules.cs:43` |
...
`PlayerIntent` 定义见 `PlayerIntent.cs:6`，由遭遇步骤从 `InputCommand` 构造。
