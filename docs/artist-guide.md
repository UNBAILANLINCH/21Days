...
1. **CI 跑在 Linux 上**（`.github/workflows/` 里是 `ubuntu-latest`），Linux 区分大小写，
   `Hero.png` 和 `hero.png` 在你的 Windows 上是同一个文件，在 CI 上是两个——本地好好的，打包就找不到图。
2. 这个工程已经踩过**两次** Windows 中文编码的坑（记录在 `ai-docs/pitfalls.md`，都是 GBK 与 UTF-8 打架）。
   中文文件名走命令行、走打包脚本、走 CI 时是同一类风险，能避就避。
