...
> **分两次出，中间停一次**（PRP 三道门之三，见 [`PRP/README.md`](../../PRP/README.md#三道门)）：
> 先只产 `prp.md`，把架构决策摆给用户，**等点头**；拿到点头再产 `tasks.md`。
> 两样一口气端上来，人只会审任务不审架构——等发现架构不对，任务已经做了一半。
> 用户说「一起出」时照办，但在回复里说明这一步跳过了设计评审。
...
## 产出 `PRP/$1/tasks.md`（**拿到设计点头之后才做这一步**）

有序、文件级、可逐项打勾。每项写清：**动哪个文件路径 → 做什么 → 产出什么 → 派给哪档模型**。

拆完对一遍验收覆盖：`prd.md` 的每条验收标准，至少有一项任务负责让它成立；
对不上的当场说出来——要么补任务，要么那条验收标准本就不该在。
